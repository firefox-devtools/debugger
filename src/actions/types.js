...
import type {Source, Breakpoint, Location, SourceText, Frame, Why} from "../types";
...
type BreakpointResult = { actualLocation: Location, id: string, text: string };
...
type SourceAction = { type: "ADD_SOURCE", source: Source } |
  { type: "SELECT_SOURCE", source: Source, line?: number, tabIndex?: number } |
  ...
  } |
  {
    ...
    status: AsyncStatus,
    error: string,
    value: { isPrettyPrinted: boolean, sourceText: SourceText, frames: Frame[] }
  } |
...
type UIAction = { type: "TOGGLE_FILE_SEARCH", searchOn: boolean } |
  ...
  { type: "TOGGLE_PANE", position: panelPositionType, paneCollapsed: boolean };

type PauseAction = { type: "BREAK_ON_NEXT", value: boolean } |
  { type: "RESUME", value: void } |
  {
    type: "PAUSED",
    pauseInfo: { why: Why, frame: Frame, isInterrupted?: boolean },
    frames: Frame[],
  ...
  { type: "ADD_EXPRESSION", id: number, input: string, value: string } |
  {
  ...
  { type: "UPDATE_EXPRESSION", id: number, input: string } |
  { type: "DELETE_EXPRESSION", id: number };
 ...
 * @static
 */
export type Action = SourceAction |
  BreakpointAction |
  PauseAction |
  { type: "NAVIGATE" } |
  UIAction;