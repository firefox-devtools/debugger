...
function getValue(expression) {
  ...
  if (!value) {
    return { path: expression.from, value: "<not available>" };
  }

  if (value.exception) {
    return { path: expression.from, value: value.exception };
  }

  return { path: value.result.actor, value: value.result };
}
...
const Expressions = React.createClass({
  ...
  inputKeyPress(e, { id }) {
    ...
    const expression = { input: e.target.value, id };
  ...
  updateExpression(expression, { depth }) {
    ...
    this.props.updateExpression({ id: expression.id, input: expression.input });
  },
  ...
  renderExpression(expression) {
    ...
    const root = { name: expression.input, path, contents: { value } };

    return dom.div(
      { className: "expression-container", key: path },
      ObjectInspector({
        roots: [ root ],
