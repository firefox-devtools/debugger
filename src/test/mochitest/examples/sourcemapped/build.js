...
const tests = fs.readdirSync(fixturesFolder).map(name => {
  if (name[0] === ".") return;
...
(async function() {
    ...
    -->${fixtures
      .map(
        ({ name, testFnName, scriptPath }) =>
          `\n    <script src="${path.relative(examplesDir, scriptPath)}"></script>` +
  ...
  const assets = fixtures.reduce(
    (acc, { assets }) => acc.concat(assets),
    [],
  );
