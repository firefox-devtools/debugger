...
const { getSources, getSourceById, getPause, getBreakpoints, getBreakpointsDisabled } = require("../selectors");
...
function _getBreakpoints(state) {
  return getBreakpoints(state).map(bp => {
    ...
    const source = getSourceById(sources, bp.location.sourceId);
